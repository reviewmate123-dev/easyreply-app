...
export async function POST(request: NextRequest) {
  ...
  'anonymous';
    ...
    try {
      ...
      const newQuestionsCount = result?.newQuestions || 0;
      return NextResponse.json({ 
        success: true, 
        newQuestions: newQuestionsCount,
        message: newQuestionsCount > 0 
          ? `Found ${newQuestionsCount} new questions` 
      ...
      if (syncError.message?.includes('rate limit')) {
        ...
        return NextResponse.json({ 
          error: syncError.message || 'Failed to sync questions' 
        }, { status: 500 });
      }
    }
